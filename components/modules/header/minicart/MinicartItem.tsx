...
const MinicartItem = ({ item }: { item: ICartItem }) => {
  const { deleteSpinner, increasePriceWithAnimation, decreasePriceWithAnimation, count, setCount, animatedPrice, handleDeleteCartItem } = useCartItemAction(item)
    ...
    <>
      ...
      <div className='cart-list__item__inner'>
        ...
        <div className='cart-list__item__bottom'>
          <span className='cart-list__item__price'>{formatPrice(animatedPrice)} ₽</span>
        </div>
      </div>
    </>
  )
