...
export const setChosenPickupAddressData =
  order.createEvent<Partial<IParkhomenkoAddressData>>()
...
export const getParkhomenkoOfficesByCityFx = order.createEffect(
  async ({ city, lang }: IGetParkhomenkoOfficesByCityFx) => {
    try {
      ...
      const ParkhomenkoData = await api.get(
...
export const $ParkhomenkoDataByCity = order
  ...
  .on(getParkhomenkoOfficesByCityFx.done, (_, { result }) => result)
...
export const $shouldLoadParkhomenkoData = order
  .createStore(false)
  .on(setShouldLoadParkhomenkoData, (_, value) => value)

export const $chosenPickupAddressData = order
  .createStore<Partial<IParkhomenkoAddressData>>({})
...
export const $courierAddressData = order
  .createStore<IParkhomenkoAddressData>({} as IParkhomenkoAddressData)
